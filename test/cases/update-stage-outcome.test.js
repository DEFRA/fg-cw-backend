...
import { receiveAndDeleteMessages } from "../helpers/sqs.js";
...
describe("PATCH /cases/{caseId}/stage/outcome", () => {
  ...
  it("updates stage outcome and transitions to next stage", async () => {
    // Unique ref so unrelated/late events can't satisfy this assertion.
    ...
    await vi.waitFor(
      async () => {
        const caseStatusUpdatedEvents = await receiveAndDeleteMessages(
      ...
      {
        // LocalStack + outbox + SNS->SQS delivery can be slow on CI runners.
        timeout: 60_000,
        interval: 1000,
      },
    );
