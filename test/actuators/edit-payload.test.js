...
describe("the edit fence", () => {
  ...
  it("412s a stale editor whose payload matches the one now stored", async () => {
    const doc = await aStoredInbox();
    await editInboxPayload(idOf(doc), anEdit());

    const refusal = await refusalOf(
      editInboxPayload(idOf(doc), anEdit({ revision: 0 })),
    );
  ...
  it.each([
    ["inbox", aStoredInbox, editInboxPayload, () => inbox],
    ["outbox", aStoredOutbox, editOutboxPayload, () => outbox],
  ])(
    "edits a %s row stored with payloadRevision 0 at revision 0",
    async (box, aStored, editPayload, collectionOf) => {
      const doc = await aStored({ payloadRevision: 0 });

      const { payload } = await editPayload(idOf(doc), anEdit());

      expect(payload.payloadRevision).toBe(1);
      expect((await collectionOf().findOne({ _id: doc._id })).event).toEqual(
        anEditedEvent(),
      );
    },
  );
...
describe("POST /actuators/events/outbox/{id}/payload", () => {
  ...
  it("leaves segregationRef as stored when the fields it derives from change", async () => {
    const caseRef = `CASE-${new ObjectId().toHexString()}`;
    const doc = await aStoredOutbox({
      segregationRef: `${caseRef}-WF`,
      event: {
        id: "evt-2",
        type: "cloud.defra.test.fg-cw-backend.x",
        data: { caseRef, workflowCode: "WF" },
      },
    });
    ...
    await editOutboxPayload(
      ...
      anEdit({
        payload: {
          ...doc.event,
          data: { caseRef: "EDITED", workflowCode: "WF" },
        },
      }),
    );
    ...
    expect(stored.event.data.caseRef).toBe("EDITED");
    expect(stored.segregationRef).toBe(`${caseRef}-WF`);
  });
