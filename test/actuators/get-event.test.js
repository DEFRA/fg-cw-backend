...
describe("GET /actuators/events/inbox/{id}", () => {
  ...
  it("never returns a stack stored alongside a lastError", async () => {
    ...
    expect(JSON.stringify(payload)).not.toContain("SECRET-DETAIL-STACK");
    expect(Object.keys(payload.lastError)).toEqual(["name", "message", "at"]);
  });

  it("labels a type-less inbox row unknown and never audit", async () => {
    const doc = anInboxDoc({ type: undefined });
    await inbox.insertOne(doc);
...
describe("GET /actuators/events/outbox/{id}", () => {
  it("404s for an id that does not exist", async () => {
  ...
  it("returns an audit event's own page by id, labelled", async () => {
    const doc = anOutboxDoc({
      target: "arn:aws:sns:eu-west-2:000000000000:cw__sns__audit_topic_arn",
      event: { audit: { entities: [{ entity: "CASE" }], status: "SUCCESS" } },
    });
    await outbox.insertOne(doc);
  ...
  it("labels a type-less row on another topic unknown", async () => {
    const doc = anOutboxDoc({ event: { id: "evt-no-type" } });
    await outbox.insertOne(doc);
