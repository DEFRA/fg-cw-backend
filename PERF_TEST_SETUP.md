...
- **Actions**:
  1. Clears all data from: cases, users, workflows, outbox, inbox
  2. Seeds 100 test cases with consistent data
...
db.applications.countDocuments({ caseRef: /^perf-test-/ })
# Expected: 100

# Check first few test cases
db.applications.find({ caseRef: /^perf-test-/ }).limit(5)
```
...
# View sample data
db.applications.findOne({ caseRef: "perf-test-000" })
```
...
1. Deploy `fg-cw-backend/hotfix/perf-test-seed`
...
```javascript
db.applications.deleteMany({ caseRef: /^perf-test-/ });
db.application_series.deleteMany({ caseRefs: /^perf-test-/ });
```
