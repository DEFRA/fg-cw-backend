...
describe("editFence", () => {
  it("matches a row never edited on a missing, null or 0 counter", () => {
    expect(editFence(0)).toEqual({
      status: { $in: ["DEAD_LETTER", "PURGED"] },
      payloadRevision: { $in: [null, 0] },
    });
  });
...
describe("isStaleEdit", () => {
  it("reads a row never edited as revision 0", () => {
    expect(isStaleEdit({}, 0)).toBe(false);
    expect(isStaleEdit({}, 1)).toBe(true);
  });

  it("is stale unless the revision is the row's", () => {
    expect(isStaleEdit({ payloadRevision: 2 }, 2)).toBe(false);
    expect(isStaleEdit({ payloadRevision: 2 }, 1)).toBe(true);
  });
});
