...
// The M3 guard itself. Every response schema here is declared with
// `failAction: "log"`, so a schema cannot be what keeps a stored stack off the
// wire - this rebuild is. Both the list rows and the detail go through it.
describe("toStoredLastError", () => {
  it("rebuilds the three contract keys and drops a stored stack", () => {
    expect(
      toStoredLastError({
        name: "TypeError",
        message: "boom",
  ...
  // What `toLastError` actually writes today, end to end.
  it("drops the stack of a lastError this service just recorded", () => {
    const stored = toLastError(new Error("boom"));

    expect(stored.stack).toContain("Error: boom");
    expect(Object.keys(toStoredLastError(stored))).toEqual([
      "name",
      "message",
      "at",
    ]);
  ...
  it("stays null for a row that never failed", () => {
    expect(toStoredLastError(null)).toBeNull();
    expect(toStoredLastError(undefined)).toBeNull();
  });
});
