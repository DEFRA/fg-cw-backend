...
// Revision 0 is a row never edited: the field is missing, null or 0.
export const editFence = (revision) => ({
  status: { $in: REDRIVABLE_STATUSES },
  payloadRevision: revision === 0 ? { $in: [null, 0] } : revision,
});

export const isStaleEdit = (row, revision) =>
  (row.payloadRevision ?? 0) !== revision;
