...
import { EVENT_TYPE_FIELDS } from "../audit/event-audit.js";
import {
...
describe("breakdownStages", () => {
  ...
  it("groups audit records apart from type-less anomalies when it can tell them apart", () => {
    const [, group] = stages({
...
describe("toBreakdownGroup", () => {
  ...
  it("carries the grouped audit flag through as a boolean", () => {
    expect(
      toBreakdownGroup(
        aRow({ _id: { error: "boom", type: null, audit: true } }),
      ).audit,
    ).toBe(true);
  });
