...
// What the audit clause MEANS lives in events/event-audit.js and is tested
// there; this is about where it lands in the combined filter.
describe("buildEventListFilter audit clause", () => {
  const build = (overrides) =>
    buildEventListFilter({
      eventIdField: "event.id",
      targetField: "target",
  ...
  it("removes the rows addressed at the audit topic when excluding", () => {
    expect(build({ audit: "exclude" })).toEqual({
      target: { $not: /(^|:)cw__sns__audit_topic_arn$/ },
    });
  });

  it("removes nothing when including", () => {
    expect(build({ audit: "include" })).toEqual({});
  });

  it("removes nothing when the caller said nothing", () => {
    expect(build({})).toEqual({});
  });

  it("AND-s with every other filter rather than replacing it", () => {
    const filter = build({
      status: "DEAD_LETTER",
      error: "boom",
      audit: "exclude",
  ...
  it("adds nothing for a box with no target field", () => {
    expect(
      buildEventListFilter({
        eventIdField: "messageId",
        targetField: null,
