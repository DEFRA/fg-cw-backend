import Joi from "joi";
import { AUDIT_EXCLUDE, AUDIT_MODES } from "../audit/event-audit.js";
import { EVENT_STATUSES } from "../counts/status-counts.js";
...
const MAX_Q = 200;
const MIN_ERROR = 1;
// The same ceiling `lastError.message` is stored under
// (events/models/last-error.js):
...
const selection = () => ({
  ...
  // Matched per box - see events/filter/event-list-filter.js.
  q: Joi.string()
    .trim()
    .min(MIN_Q)
    .max(MAX_Q)
  ...
  // events/audit/event-audit.js. Detail endpoints do not take it: an audit event's
