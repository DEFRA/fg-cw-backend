...
import { breakdown } from "../repositories/outbox.repository.js";
...
describe("breakdownOutboxUseCase", () => {
  ...
  it("never applies the error filter it is given", async () => {
    await breakdownOutboxUseCase({ error: "boom" });
...
describe("breakdownOutboxUseCase audit", () => {
  it("passes the resolved audit mode to the repository", async () => {
    await breakdownOutboxUseCase({ audit: "include" });

    expect(breakdown.mock.calls.at(-1)[0].audit).toBe("include");

    await breakdownOutboxUseCase({ audit: "exclude" });
