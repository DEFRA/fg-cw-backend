...
describe("caseService", () => {
  ...
  describe("createCase", () => {
    it("should call createCase on caseRepository with correct arguments", async () => {
      ...
      const mockResponse = { _id: insertedId, ...caseData1 };

      caseRepository.createCase.mockResolvedValue(mockResponse);
  ...
  describe("findCases", () => {
    it("should call findCases on caseRepository and return the cases", async () => {
      ...
      const result = await caseService.findCases(mockDb);

      expect(caseRepository.findCases).toHaveBeenCalledWith(mockDb);
  ...
  describe("getCase", () => {
    it("should call getCase on caseRepository with correct arguments", async () => {
      ...
      caseRepository.getCase.mockResolvedValue(mockResponse);
