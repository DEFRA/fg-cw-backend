...
describe("inbox model lastError", () => {
  ...
  it("records the caught error's name and message on markAsFailed", () => {
    ...
    expect(obj.lastError).toEqual({
      ...
      // Stored for diagnosis; `toStoredLastError` never serves it.
      stack: expect.stringContaining("TypeError: no handler found"),
    });
  });
...
describe("inbox model expireAt", () => {
  ...
  it("round-trips through a document", () => {
    ...
    const stored = Inbox.fromDocument(event.toDocument());
  ...
  it("reads back as null off a row written before the field existed", () => {
    const { expireAt, ...legacy } = Inbox.createMock().toDocument();
