...
describe("inbox.repository findPage", () => {
  ...
  it("labels a type-less inbox row unknown rather than audit", async () => {
    const opts = await callFindPage({ audit: "exclude" });
    const row = opts.mapDocument(aDoc({ type: undefined }));
  ...
  it("projects only the generic fields", async () => {
    ...
    expect(opts.project).not.toHaveProperty("traceparent");
  });
  ...
  it("returns null rather than undefined for absent optional fields", async () => {
    ...
    expect(row).toEqual({
      ...
      eventId: null,
      // never null, and never "audit": nothing addressed an inbox row anywhere
      type: "unknown",
      source: null,
      status: InboxStatus.PUBLISHED,
