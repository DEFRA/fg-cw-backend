...
export const toListRow = (doc) => ({
  ...
  // Never null. An inbox row carries no destination, so it can never be an
  // audit record: a type-less one is always "unknown". See
  // events/event-audit.js.
  type: typeLabels(orNull(doc.type), false).type,
...
export const findPage = ({
  ...
  paginate(db.collection(collection), {
    ...
    // `segregationRef` and `traceparent` are still SEARCHABLE - `q` matches
...
export const findDetailById = async (id) => {
  ...
  return doc ? toDetailDocument(doc, MAX_RETRIES, "inbox") : null;
};
