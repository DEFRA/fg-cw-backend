...
describe("boxBreakdownResponseSchema", () => {
  ...
  // Required, so a mapping gap fails a test rather than quietly turning
  // every audit group into an unknown one.
  it("accepts an audit-flagged group with no stored type", () => {
    expect(
      validate([aGroup({ type: null, audit: true })]).error,
    ).toBeUndefined();
  });

  it("requires the audit flag", () => {
    const { audit, ...group } = aGroup();
