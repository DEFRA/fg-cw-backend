...
import { parkInboxEventUseCase } from "./park-inbox-event.use-case.js";
...
describe("parkInboxEventUseCase", () => {
  ...
  it("answers with the parked row and its retry cap", async () => {
    ...
    const row = await parkInboxEventUseCase(ID, { reason: "poison" });
  ...
  it("is a 404 when there is no such row", async () => {
    ...
    await expect(
      parkInboxEventUseCase(ID, { reason: "poison" }),
  ...
  it("only reads the status on the failure path", async () => {
    ...
    await parkInboxEventUseCase(ID, { reason: "poison" });
