...
const notFound = (id) => Boom.notFound(`Inbox event "${id}" not found`);
...
const refusalOf = (id, row, revision) => {
  ...
  if (!REDRIVABLE_STATUSES.includes(row.status)) {
    logger.warn(`Refused an edit of inbox event "${id}" - it is ${row.status}`);

    return editConflict("Inbox", id, row.status);
  }

  if (isStaleEdit(row, revision)) {
    logger.warn(`Refused a stale edit of inbox event "${id}"`);

    return staleEdit("Inbox", id);
  }
...
const editInboxEventPayload = async (
  { id, by, payload, note, revision },
  session,
) => {
  logger.info(
    `Editing the payload of inbox event "${id}" from revision ${revision} for ${by}`,
  );
  ...
  if (
    ...
    logger.info(
      `Finished: Editing the payload of inbox event "${id}", now at revision ${revision + 1}`,
    );
...
// the note do not.
export const editInboxEventPayloadAuditBuilder = (
...
export const editInboxEventPayloadUseCase = (command) =>
  withTransaction((session) =>
    withAudit(editInboxEventPayload, editInboxEventPayloadAuditBuilder)(
      command,
      session,
    ),
  );
