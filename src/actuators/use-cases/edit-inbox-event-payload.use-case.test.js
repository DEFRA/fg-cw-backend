...
describe("editInboxEventPayloadUseCase", () => {
  ...
  it("writes the fenced update with the edit and the stored original", async () => {
    findEditableById.mockResolvedValue({ ...A_ROW, payloadRevision: 2 });
  ...
  it("reads the row once on the happy path", async () => {
    await editInboxEventPayloadUseCase(aCommand());

    expect(findEditableById).toHaveBeenCalledTimes(1);
  });
  ...
  it.each(["COMPLETED", "RESUBMITTED", "PUBLISHED"])(
    ...
    async (status) => {
      findEditableById.mockResolvedValue({ ...A_ROW, status });
  ...
  it("409s rather than 422s a row in another status", async () => {
    findEditableById.mockResolvedValue({ ...A_ROW, status: "COMPLETED" });

    expect((await refusedWith(aCommand({ payload: STORED }))).statusCode).toBe(
      409,
    );
  });

  it("412s rather than 422s a stale editor sending the current payload", async () => {
    findEditableById.mockResolvedValue({ ...A_ROW, payloadRevision: 1 });
  ...
  it("404s when the row went between the read and the update", async () => {
    ...
    findEditableById.mockResolvedValueOnce(A_ROW).mockResolvedValueOnce(null);

    expect((await refusedWith(aCommand())).statusCode).toBe(404);
  });

  it("412s when the fence matched nothing though the row still reads as current", async () => {
    editPayloadById.mockResolvedValue(false);

    expect((await refusedWith(aCommand())).statusCode).toBe(412);
  });

  it("reads the refused row again inside the transaction", async () => {
    editPayloadById.mockResolvedValue(false);

    await refusedWith(aCommand());

    expect(findEditableById).toHaveBeenNthCalledWith(2, ID, SESSION);
  });
...
describe("editInboxEventPayloadUseCase audit", () => {
  ...
  it("records a refused edit as a FAILURE with its reason and no paths", async () => {
    findEditableById.mockResolvedValue({ ...A_ROW, payloadRevision: 5 });
  ...
  it.each([
    ["NOT_FOUND", null],
    ["NOT_EDITABLE", { ...A_ROW, status: "COMPLETED" }],
  ])("records the reason %s on a FAILURE", async (reason, row) => {
    findEditableById.mockResolvedValue(row);
