...
export const editOutboxEventPayloadRoute = {
  method: "POST",
  path: "/actuators/events/outbox/{id}/payload",
  options: {
    description:
      "Replace the payload of one DEAD_LETTER or PURGED outbox event, leaving its status alone. Names the operator in `by`, which the edit is audited under and so is required. 200 with the new payloadRevision and the changed paths; 400 without an operator, or when the payload is not a JSON object or the note is missing; 404 when there is no such event; 409 naming the row's status when it is in any other; 412 when it was edited since `revision`; 422 with a `reason` of TOO_LARGE, UNCHANGED or DOLLAR_KEY.",
    auth: "public-api",
    tags: ["api", "public-api"],
    plugins: {
  ...
  async handler(request) {
    const { payloadRevision, changedPaths, changedPathsTruncated } =
      await editOutboxEventPayloadUseCase({
