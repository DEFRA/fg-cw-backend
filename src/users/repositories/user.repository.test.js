...
describe("save", () => {
  ...
  it("throws Boom.conflict when email exists", async () => {
    ...
    await expect(save(user)).rejects.toThrow(
      Boom.conflict(`User with the same email already exists`),
    );
  });
...
describe("update", () => {
  ...
  it("throws Boom.notFound when user is not found", async () => {
    ...
    await expect(update(userId, {})).rejects.toThrow(
      Boom.notFound(`User with id ${userId} not found`),
    );
  });
